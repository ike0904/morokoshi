...
### v2.0.6 (2026-08-24) ← 最新
- 【バグ1修正】NSF ch solo 時に INIT バースト後の長い無音が保持されない問題を修正（v2.0.5 の修正を差し替え）
  - 原因: DLL の gme_clear_blip_buffer は Multi_Buffer をクリアするが NES APU オシレーター状態はリセットされない
    → 最初の gme_play() フレームで INIT レジスタのまま ch が発音し _LEAK_S を越えると `nz0=0` になり cleanup が何もしなかった
  - v2.0.5 の誤り: `_GAP_MIN=500ms`（バースト後の無音下限）が大きすぎて全件スキップしていた
  - 修正: `_GAP_MIN` を撤廃し、バーストが ≤300ms で終わればその終端（最初の無音サンプル）までゼロ化するシンプルな方針に変更
  - 対象ログ: `NSF INIT burst: zeroing 0..Xs` が出れば修正が適用されている

### v2.0.5 (2026-08-24)
- 【バグ1修正試行】v2.0.5 の対応は _GAP_MIN=500ms の設定ミスで機能せず → v2.0.6 で差し替え
