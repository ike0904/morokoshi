...
| **Rew/FF** | 早戻し・早送りの幅（秒）。Tempo・Beat・Barから自動計算されます。ドラッグ・ホイール・直接入力で変更するとTempoが逆算されます。**シングルクリック**するとA<->B間の秒数をそのままコピーしてTempoを逆算します。 |
...
| **テンポ検出の精度** | テンポ検出はオンセット検出とビートトラッキングによる推定です。複雑なリズムの曲や一定でないテンポの曲では、誤った値が検出される場合があります。 |
...
> 💡 **キーボードの種類による差異**: JISキーボードの最上段は13キー（ch12=`^`、ch13=`¥`）。USキーボードは最上段が12キーしかないため ch12=`=`、ch13=`\`（Enterキーの上）に割り当てられます。
...
| **v1.8.0** | 2026/7/17 | **ESCキー**で再生を一時停止できるようになりました。<br>タスクバーのアイコンをもろこしマークに修正しました。<br>マーカーのずれを修正しました。 |
...
| **Rew/FF** | Rewind/fast-forward distance in seconds. Auto-calculated from Tempo, Beat, and Bar. Can also be set directly; doing so back-calculates Tempo. **Single-click** to copy the A<->B duration directly and back-calculate Tempo. |
...
| **Tempo detection accuracy** | Tempo detection uses onset detection and beat tracking and is an estimate. It may return incorrect values for songs with complex rhythms or non-constant tempos. |
...
> 💡 **Key layout note**: On a US keyboard, the top row has 12 typeable keys (`1`–`=`); channel 12 maps to `=`. Channel 13 maps to `\` (the key above Enter, not in the number row). On a JIS keyboard, the top row has 13 keys; ch12 = `^`, ch13 = `¥`.
...
| **v1.8.0** | 2026/7/17 | Added **ESC key** to pause playback.<br>Fixed taskbar icon to show the Morokoshi icon.<br>Fixed a marker position offset bug. |
