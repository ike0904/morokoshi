...
<p style="font-size: 11pt; color: #888; margin: 0 0 50px;">v1.7.0</p>
...
> 💡 **キーボードの種類による差異**: JISキーボードの最上段は13キー（ch12=`^`、ch13=`¥`）。USキーボードは最上段が12キーしかないため ch12=`=`、ch13=`\`（Enterキーの上）に割り当てられます。ch13 が必要なゲームはほぼないため実用上の問題はありません。
...
> 💡 **Key layout note**: On a US keyboard, the top row has 12 typeable keys (`1`–`=`); channel 12 maps to `=`. Channel 13 maps to `\` (the key above Enter, not in the number row) — games requiring ch13 are rare. On a JIS keyboard, the top row has 13 keys; ch12 = `^`, ch13 = `¥`.
